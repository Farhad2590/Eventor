...
const EventCard = () => {
  const [allEvents, setAllEvents] = useState([]); // Store complete dataset
  ...
  useEffect(() => {
    ...
    if (filters.search) {
      ...
      results = results.filter(event => 
        ...
        (event.features && event.features.some(feature => 
    ...
    if (filters.minTeamSize) {
      ...
      results = results.filter(event => event.photography_team_size >= minTeamSize);
    }
    if (filters.maxTeamSize) {
      const maxTeamSize = Number(filters.maxTeamSize);
      results = results.filter(event => event.photography_team_size <= maxTeamSize);
    }

    if (filters.videography !== "") {
      const wantsVideography = filters.videography === "true";
      results = results.filter(event => event.videography === wantsVideography);
    }
  ...
  const handleConfirmDate = () => {
    axiosSecure
      .post("/confirmEvents", confirmDate)
      .then((res) => {
                    ...
                    <div className="p-6 flex-1">
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          className="flex-1 bg-[#179ac8] hover:bg-[#147a9e] text-white py-3 px-4 rounded-xl font-medium transition-all duration-200 hover:shadow-lg flex items-center justify-center gap-2"
                        >
                          <FiCalendar size={18} />
                          Book Now
                        </button>
                        <Link
                          to={`/event/${event?._id}`}
                          className="flex-1 border-2 border-[#179ac8] text-[#179ac8] hover:bg-[#179ac8] hover:text-white py-3 px-4 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-2"
                        >
                          <FiStar size={18} />
                          Customize
                        </Link>

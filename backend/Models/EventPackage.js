...
const eventPackageSchema = new mongoose.Schema(
  {
    ...
    images: {
      ...
      validate: [
        ...
        {
          validator: function(images) {
            return images.every(url => /^https?:\/\/.+\.(jpg|jpeg|png|webp|gif)$/i.test(url));
          },
...
eventPackageSchema.pre('save', function(next) {
  if (this.isNew || this.isModified('category') || this.isModified('features')) {
    const autoTags = [];
...
const EventPackage = mongoose.model("events", eventPackageSchema);
